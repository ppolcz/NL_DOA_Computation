...
properties (Hidden)
    subsvars__
    subsvars2symvars_fh__
    symvars2subsvars_fh__
end
...
properties (Dependent)
    ...
    % pl.    @([x1;p1;p2;x2;x3]) [x1;x2;p1;p2]
    % vagyis @(v) [v(1);v(4);v(2);v(3)]
    subsvars2symvars_fh    
    subsvars2lfrvars_fh
    
    % pl.    @([x1;x2;p1;p2]) [x1;p1;p2;x2;0] (0. ami nem szerepel benne)
    symvars2subsvars_fh
    lfrvars2subsvars_fh
end
...
methods
    ...
    % symvars == lfrvars
    function lfrvars = get.lfrvars(pLFR)
        lfrvars = pLFR.symvars;
    end
    ...
    % 2020.05.19. (május 19, kedd), 20:46
    function pLFR = set.subsvars(pLFR,val)
        % if strcmp(pLFR.lfrtbx_obj,'[EMPTY]')
        %     return
        % end
        % 
        % if strcmp(val,'[EMPTY]')
        ...
        % end
        % 
        % if iscell(val) && isa(val{1},'lfr')
        %     vars_cell = cellfun(@(v) {sym(v.blk.names{1})},val); 
        %     val = vertcat(vars_cell{:});
        % end
                
        pLFR.subsvars__ = val;
        
        % handle missing substitutional variables in the lfr variables
        missing = setdiff(pLFR.subsvars__, pLFR.symvars(:));        
        ZERO__ = sym('ZERO__');
        subsvars0 = subs(pLFR.subsvars__,missing,missing*0 + ZERO__);
        
        if isempty(pLFR.symvars)
            % subs > []
            pLFR.subsvars2symvars_fh__ = @(x) [];
        
            % [] > subs
            tmp_symvars2subsvars_fh = matlabFunction(subsvars0(:), 'vars',{ZERO__});        
            pLFR.symvars2subsvars_fh__ = @(x) tmp_symvars2subsvars_fh(zeros(1,size(x,2)));
        else
            % subs > lfr
            pLFR.subsvars2symvars_fh__ = matlabFunction(pLFR.symvars(:),'vars',{pLFR.subsvars__});

            % lfr > subs
            tmp_symvars2subsvars_fh = matlabFunction(subsvars0(:), 'vars',{pLFR.symvars(:) , ZERO__});        
            pLFR.symvars2subsvars_fh__ = @(x) tmp_symvars2subsvars_fh(x,zeros(1,size(x,2)));
        end
    ...
    function subsvars = get.subsvars(pLFR)
        subsvars = pLFR.subsvars__;
    end
    
    function subsvars2symvars_fh = get.subsvars2symvars_fh(pLFR)
        subsvars2symvars_fh = pLFR.subsvars2symvars_fh__;
    end
    
    function subsvars2lfrvars_fh = get.subsvars2lfrvars_fh(pLFR)
        subsvars2lfrvars_fh = pLFR.subsvars2symvars_fh__;
    end
    
    function symvars2subsvars_fh = get.symvars2subsvars_fh(pLFR)
        symvars2subsvars_fh = pLFR.symvars2subsvars_fh__;
    end
    
    function lfrvars2subsvars_fh = get.lfrvars2subsvars_fh(pLFR)
        lfrvars2subsvars_fh = pLFR.symvars2subsvars_fh__;
    end
...
methods (Access = private)
	...
	pLFR = generate_LFR(pLFR,varargin)
...
methods (Access = public)
    ...
    % Random sampling.
    [sampled,subs_samples,lfr_samples,samples_good] = dbsample(syslfr,varargin)
    ...
    JLFR = jacobian(pLFR,p_cell,subsvars)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 %%% Simplified operator wrappers
  %  2020.03.29. (március 29, vasárnap), 12:55
    ...
    function H = uminus(G)
        H = plfr(uminus(G.lfrtbx_obj),G.subsvars(:));
    end
    ...
    function H = inv(G)
        H = plfr(inv(G.lfrtbx_obj),G.subsvars(:));
    end
