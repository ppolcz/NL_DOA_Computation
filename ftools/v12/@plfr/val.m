...
% 2020.05.19. (május 19, kedd), 22:47 (`cellout' functionality)
if ischar(varargin{1})
    cellout = strcmp(varargin{1},'cellout');
...
if CASE == 0
    error('Arguments are not good: size(arg1) = [%d,%d], numel(varargin) = %d, np = %d.', size(varargin{1},1), size(varargin{1},2), numel(varargin), pLFR.np)
end
...
% 2020.05.19. (május 19, kedd), 22:49 (`cellout')
if cellout
    if nargout == numel(val)
        varargout = val;
    else
        varargout{1} = val;
    end
