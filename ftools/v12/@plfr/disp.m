...
%}
    ...
    if ~isempty(G.names)
        ...
        varstr = sprintf('  (%s)', stringify(G.vars));
        varstr = strrep(varstr,'  (1,','1,(');
        
        
        lengthdiff = numel(varstr) - numel(fhargs);
        
        % fprintf('Names, dimensions:    %s \n', names)
        fprintf('Blocks (symvars|lfrvars):       %s %s[ns=%d]\n', varstr, repmat(' ',[max(0,-lengthdiff) 1]), G.ns)
        fprintf('Function handle args (subsvars):%s %s[np=%d]\n', fhargs, repmat(' ',[max(0,+lengthdiff) 1]), G.np)
