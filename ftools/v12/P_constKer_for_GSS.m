function [constKer,M,samples,sampleserr,error_] = P_constKer_for_GSS(matlfr,varargin)
%% P_constKer_for_GSS
%
...
if isa(matlfr,'plfr')
    matlfr = matlfr.lfrtbx_obj;
end

if isa(matlfr,'lfr')
    try
        matlfr = lfr2gss(matlfr);
    catch e
        if size(matlfr,'order') == 0
            matlfr = gss(matlfr.d);
        else
            error(e)
        end
...
while ~isempty(find(size(matlfr) == N,1))
    N = N + 1;
end
...
while true
    ...
    ZERO_gss = mingss(matlfr*constKer);
    ...
    [M_sampled,new_samples] = dbsample(matlfr,N);
